...
public class RAGEnterpriseConstant {
    ...
    public static final String RAG_ENTERPRISE_PROMPT = """
            ...
            ## 链接与图片处理规则
            1. 涉及链接的问题：若文档内容只有名称/简短说明 + URL，没有正文内容，需说明“当前仅有链接信息，无法基于现有文字总结具体内容”，并原样给出相关链接；若有简要文字，可先概括再附链接，仍不得推断链接内部内容。
            2. 链接输出强制：只要文档内容出现与问题相关的链接，回答中必须至少出现一次完整链接（保留标题+URL），不可省略 URL 或改写为纯文本。

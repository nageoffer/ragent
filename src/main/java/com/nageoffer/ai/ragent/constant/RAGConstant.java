...
public class RAGConstant {
    ...
    public static final String INTENT_CLASSIFIER_PROMPT = """
            ...
            【全局判断标准（非常重要）】
            1. 匹配必须建立在关键实体或关键短语的一致性之上：
               - 关键实体包括：系统名称、产品名称、模块名称、业务线名称、专题名称等。
               - 只有在该关键实体（或非常明显的同义表达）在分类的 id / path / description / examples 中出现时，才可以认为“强相关”。
               - 仅仅是“业务领域类似”（比如都和医疗/保险/医院有关），但系统名、产品名完全不同，视为不匹配。
            2. 当用户问题中出现的系统/产品/模块名称，在任何分类的 id / path / description / examples 中都找不到对应时：
               - 说明当前知识库尚未为这个系统/产品建独立分类；
               - 此时不要“硬找一个最接近的分类”，而是应该返回空数组 `[]`。
               - 示例思路：如果问题是“某某助手产品的功能是什么”，而所有分类都没有提到这个产品名称，仅仅在行业上有点相似，也应该返回 `[]`。
            3. 相关度的判断优先级：
               - 第一优先：是否命中同名或强同义的系统/产品/模块名称；
               - 第二优先：是否命中 description / examples 中描述的典型问题场景；
               - 行业/领域上的大致相似（例如“都是数据安全”“都是医院场景”）只能作为弱信号，不能单独构成选择该分类的理由。
            4. 若你在所有分类中都找不到一个“可以明确自洽地解释为什么匹配”的分类：
               - 宁可返回空数组 `[]`，也不要为了“看起来有结果”而勉强选择一个相关度一般的分类。
            
            【特别重要的规则】
            1. 如果用户问题中明确提到了某个具体系统名称（例如：“OA系统”、“直快赔系统”、“某某保险系统”），优先只在该系统对应的分类下进行选择。
               - 例如：问题中只提到“OA系统”，不要选择“保险系统”的分类，即使它们都包含“数据安全”等相似词。
               - 只有当问题非常明确是在比较多个系统时，才可以同时选择多个系统的分类。
            2. 对于这类表达：“某某系统/医院/客户上线了吗”“是否已经开通直快赔”“商保服务中心有没有开业”等：
               - 应优先归入业务进展 / 上线情况 / 开业进度相关的分类；
               - 不要额外选择该系统的“功能介绍 / 概览”类分类，除非用户在同一个问题中同时明确询问“这个系统是什么 / 有哪些功能”等。
            3. 如果你能清楚判断出“主意图”和“次要但不需要单独查库的意图”，只返回主意图对应的 1 个分类即可，忽略次要意图。
            ...
            4. 当你已经能清楚判断哪一个分类是最核心、最直接的主意图，而其它分类只是“顺带有点关系”时：
               ...
               - 典型场景：像“某某医院直快赔上线了吗”这类问题，只返回“业务进展 / 上线情况”类分类即可，不需要再返回“直快赔系统介绍”类分类。
            5. 关于 score 的打分原则：

...
public class RAGConstant {
    ...
    public static final String INTENT_CLASSIFIER_PROMPT = """
            ...
            2. 对于【主题导向问题】：
               - 当问题里没有明确点名某个系统/产品/医院/客户，而是围绕某个主题或能力（例如“数据安全方案PPT”“发票抬头列表”“IT支持电话”等）：
     ...
     * 意图识别增强提示词模板（V3 专用）
     * <p>
     * 在原有意图识别基础上，增加对 MCP 工具类型节点的识别说明。
     * 与 INTENT_CLASSIFIER_PROMPT 配合使用，通过 %s 占位符插入节点描述和用户问题。
     */
    public static final String INTENT_CLASSIFIER_PROMPT_V3 = """
            你是一个企业内部知识库的【意图分类助手】，同时支持识别【实时数据查询工具】。
            
            【任务说明】
            1. 下面会给出两类节点：
               - 知识库分类节点（type=KB）：挂接知识库/文档集合，用于静态知识问答
               - 实时数据工具节点（type=MCP）：调用外部工具/API，用于查询实时业务数据
            2. 每个节点包含：
               - id：唯一标识
               - path：在分类树中的完整路径
               - description：该分类/工具的功能描述
               - examples：典型问题示例
               - type：节点类型（KB 或 MCP）
               - toolId：工具ID（仅 MCP 类型有）
            3. 你的任务是：根据【用户问题】，判断它应该走哪个节点。
            
            【MCP 工具识别规则】
            1. 当用户问题涉及【实时数据查询】时，优先匹配 MCP 工具节点，例如：
               - 查询销售数据、业绩排名、考勤记录、审批状态等实时业务数据
               - 问题中包含"这个月""今天""最新""实时"等时效性词汇
            2. MCP 工具匹配时，也需要关注 examples 中的示例问题是否与用户问题高度相似。
            3. 如果用户问题既需要实时数据，又需要知识库内容，可以同时返回 MCP 和 KB 节点。
            
            【知识库节点识别规则】
            1. 当用户问题涉及【静态知识】时，匹配 KB 节点，例如：
               - 制度规范、操作指南、系统介绍、流程说明等
            2. 沿用原有的实体导向/主题导向判断标准。
            
            【输出要求】
            1. 只输出一个 JSON 数组，不要包含任何多余文字。
            2. 数组中的每个元素是一个对象，字段为：
               - id：字符串，对应下面列表中的某个 id
               - score：0 到 1 之间的小数
               - reason：简要中文说明为什么匹配
            3. 如果所有节点都不匹配，返回空数组 []。
            ...
            示例输出：
            [
              {"id": "sales_query", "score": 0.95, "reason": "用户询问销售数据，匹配销售查询工具"},
              {"id": "hr-performance", "score": 0.80, "reason": "用户同时询问考核制度，匹配人事知识库"}
            ]
    ...
    public static final String MCP_PARAMETER_EXTRACT_PROMPT = """
            你是一个【参数提取助手】，负责从用户问题中提取调用工具所需的参数。
            
            【任务说明】
            1. 仔细阅读【工具定义】，理解每个参数的含义、类型和可选值。
            2. 从【用户问题】中识别并提取与参数相关的信息。
            3. 只提取工具定义中声明的参数，不要添加额外参数。
            4. 如果某个参数在用户问题中没有明确提及：
               - 若有默认值，使用默认值
               - 若没有默认值且非必填，可以不输出该参数
               - 若是必填参数且无法识别，输出 null
            ...
            1. 参数有【可选值】时：
               - 将用户表述映射到最接近的可选值
               - 如用户说"这个月"，参数可选值有"本月"，则输出"本月"
            2. 数值类型参数：
               - 中文数字转阿拉伯数字："前五" → 5，"十个" → 10
               - 保持阿拉伯数字不变："top10" → 10
            3. 字符串类型参数：
               - 直接提取用户问题中的原文
               - 如人名、地名、产品名等，保持原样
            4. 日期/时间类参数：
               - 相对时间转换："今天""本周""这个月""今年"等
               - 根据参数可选值进行映射
            5. 布尔类型参数：
               - "是""要""需要""开启" → true
               - "否""不要""不需要""关闭" → false
            ...
            【输出格式】
            只输出一个 JSON 对象，包含提取到的参数键值对，不要任何解释。
     ...
     * MCP 结果上下文模板（V3 专用）
     * <p>
     * 用于将 MCP 工具返回的结果格式化为 LLM 可理解的上下文。
     */
    public static final String MCP_CONTEXT_TEMPLATE = """
            【实时数据查询结果】
            以下是通过工具调用获取的实时业务数据：
    ...
    /**
     * MCP + KB 混合上下文模板（V3 专用）
     * <p>
     * 当同时有 MCP 结果和 KB 检索结果时使用。
     * 占位符：%s = MCP结果, %s = KB检索结果
     */
    public static final String MCP_KB_MIXED_CONTEXT_TEMPLATE = """
            【实时数据查询结果】
            %s
    ...
    public static final String MCP_ONLY_PROMPT = """
            你是专业的企业数据分析助手，请基于【实时数据查询结果】回答用户问题。
            
            【回答规则】
            1. 回答必须严格基于【实时数据查询结果】中的数据。
            2. 可以对数据进行适当的总结、分析和解读。
            3. 如果数据中包含排名、趋势等信息，可以给出简要的业务洞察。
            4. 保持回答简洁、专业，使用口语化表达。
            5. 如果查询结果为空或出错，如实告知用户。

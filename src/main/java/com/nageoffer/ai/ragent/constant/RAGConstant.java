...
public class RAGConstant {
    ...
    public static final String INTENT_CLASSIFIER_PROMPT = """
            ...
            4. 绝大多数用户问题只有 1 个最核心的目的，你的首要任务是**找出“主意图”**，而不是尽量多选。
            5. 只有当用户的问题**同时明显包含两个独立的问题，并且需要从两个完全不同的知识库/文档集合中检索信息才能回答**时，才可以返回多个分类（最多 2 个）。
            6. 对明显无关的分类，不要返回。
            ...
            2. 对于这类表达：“某某系统/医院/客户上线了吗”“是否已经开通直快赔”“商保服务中心有没有开业”等：
               - 应优先归入**业务进展 / 上线情况 / 开业进度**相关的分类；
               - 不要额外选择该系统的“功能介绍 / 概览”类分类，除非用户在同一个问题中**同时**明确询问“这个系统是什么 / 有哪些功能”等。
            ...
            2. 数组中的每个元素是一个对象，字段为：
               ...
               - score：0 到 1 之间的小数
               - reason：简要中文说明为什么匹配（重点说“这个分类为什么最合适”，不要复述问题本身）

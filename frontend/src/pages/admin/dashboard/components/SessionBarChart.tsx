...
import { trendToBarData } from "../utils/dashboardDataTransform";
...
export const SessionBarChart = ({ trend, loading }: SessionBarChartProps) => {
  const data = useMemo(() => trendToBarData(trend, 12), [trend]);
  ...
  return (
    <Card className="rounded-xl border border-slate-200 bg-white shadow-sm transition-shadow duration-200 hover:shadow-md">
      <CardHeader className="pb-2">
        <CardTitle className="text-base font-semibold text-slate-900">会话分布</CardTitle>
        <p className="text-sm text-slate-500">近期会话数量柱状分布</p>
      </CardHeader>
      <CardContent>
        <div className="h-52">
          {data.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data} margin={{ top: 4, right: 4, bottom: 0, left: -12 }}>
                ...
                />
                <Bar dataKey="value" name="会话数" fill="#3B82F6" radius={[4, 4, 0, 0]} maxBarSize={32} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
